...
static void PipeWorker(ServerEntry* self) {
    ...
    while (!self->shutdown.load()) {
        // Permissive DACL (Everyone): when the app runs elevated
        // (requireAdministrator manifest), the default pipe DACL is
        // admins-only and non-elevated test clients get ERROR_ACCESS_DENIED.
        // The pipe only exists when the app opts in via
        // ui_debug_server_start(), so opening it up is safe.
        PSECURITY_DESCRIPTOR pSD = nullptr;
        ConvertStringSecurityDescriptorToSecurityDescriptorA(
            "D:(A;;GA;;;WD)", SDDL_REVISION_1, &pSD, nullptr);
