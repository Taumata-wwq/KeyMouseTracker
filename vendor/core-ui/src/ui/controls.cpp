...
// 把子树全部 rect 对齐到整数像素。布局/位移过程中字体测量、flex 均分可能留下
// 小数坐标，D2D 在亚像素下绘制圆角/描边会把 1px 描边吃得忽窄忽宽 —— 表现即
// "滚动后按钮变窄、按键刷新后恢复"。一律取到整数网格统一视觉。
static void SnapSubtreeIntPixels(Widget* w) {
    if (!w) return;
    w->rect.left   = std::floor(w->rect.left   + 0.5f);
    w->rect.top    = std::floor(w->rect.top    + 0.5f);
    w->rect.right  = std::floor(w->rect.right  + 0.5f);
    w->rect.bottom = std::floor(w->rect.bottom + 0.5f);
    for (auto& c : w->Children()) SnapSubtreeIntPixels(c.get());
}

// 沿 Y 平移整棵子树（含 root 自身）。滚动只改变 offset，子几何不变。
static void TranslateSubtree(Widget* w, float dy) {
    if (!w) return;
    w->rect.top += dy; w->rect.bottom += dy;
    for (auto& c : w->Children()) TranslateSubtree(c.get(), dy);
}
...
void ScrollViewWidget::DoLayout() {
    ...
    content_->DoLayout();
    SnapSubtreeIntPixels(content_.get());
}

void ScrollViewWidget::ApplyScrollDelta_(float oldScrollY) {
    if (!content_) return;
    /* 滚动只平移内容子树，不重跑 DoLayout：
     *
     * 布局层的 SizeHint（按钮/文本宽度）依赖 g_activeMeasureContext —— 该上下文
     * 只在 LayoutRoot 的绘制帧里有效。若在滚轮回调里重跑 content_->DoLayout()，
     * 此时 g_activeMeasureContext 为空，SizeHint 退到"无测量上下文的字符估算
     * 宽度"，按钮当场变窄；而按键刷新走 ui_page_set_json → RelayoutAllWindows →
     * LayoutRoot（有测量上下文）按真实文本宽测量，按钮又恢复 —— 这正是
     * "滚动后按钮变窄、按键记录后恢复"的根因。
     *
     * 滚动只改变 scrollY_ 偏移，子几何不变，因此平移即可。scrollY_ 一律取整为
     * 整数，增量 dy 为整数平移，不会像早期 ShiftSubtreeRects 累积亚像素；平移后
     * 再做整像素吸附兜底，与 LayoutRoot 全量重排的几何保持一致。 */
    float dy = scrollY_ - oldScrollY;
    if (dy == 0.0f) return;
    /* 方向：scrollY_ 增大 = 内容向上移动（布局基准 layout 里 content 的
     * rect.top = base - scrollY_，见 LayoutScrollView）。滚动增量平移需与之
     * 同向，因此取 -dy 上移；用 +dy 会导致滚轮与拖拽方向整体反向（内容
     * 跟着鼠标/滚轮同向跑）。 */
    TranslateSubtree(content_.get(), -dy);
    SnapSubtreeIntPixels(content_.get());
}
...
void ScrollViewWidget::OnDraw(Renderer& r) {
    ...
    // Scrollbar is also drawn in DrawTree (on top of content), not here.
}
...
bool ScrollViewWidget::OnMouseWheel(const MouseEvent& e) {
    ...
    /* 取整到整数像素：亚像素坐标平移会把子 rect 变成小数位，圆角/描边在 D2D
     * 下被画窄（滚动后按钮"变小、刷新后恢复"的根因）。整数滚动保证坐标稳定。 */
    scrollY_ = std::floor(scrollY_ + 0.5f);
    ClampScroll();
...
bool ScrollViewWidget::OnMouseDown(const MouseEvent& e) {
    if (NeedsScrollbar() && e.x >= rect.right - kBarSpace - 2 && e.x < rect.right - 2) {
        ...
        if (trackRange > 0 && (e.y < thumb.top || e.y > thumb.bottom)) {
            ...
            scrollY_ = std::floor(scrollY_ + 0.5f);   // 整数滚动，见 OnMouseWheel
            ClampScroll();
