...
struct PageState::JsCondRuntime {
    ...
    // Own watcher id (mirrored into jsEffectIds_; DisposeEffect is
    // idempotent so the double-dispose at Detach time is harmless).
    uint64_t effectId = 0;
    // Nested v-if runtimes wired while this subtree was mounted. Their
    // parent/template pointers live inside this subtree — torn down
    // recursively at unmount (see TearDownChildConds).
    std::vector<JsCondRuntime*> childConds;
...
struct PageState::JsLoopIteration {
    ...
    // v-for nested directly inside this iteration's template (v-for > v-for).
    // The compiler emits CompiledLoop records for them into subPage.loops;
    // the specs are MOVED here so JsLoopRuntime::spec (a raw pointer) stays
    // valid for the runtime's lifetime — the referenced parent widgets live
    // in the subtree owned by subPage, which outlives the runtimes.
    std::vector<CompiledLoop>                 innerLoopSpecs;
    std::vector<std::unique_ptr<JsLoopRuntime>> innerLoops;
};
...
PageState::JsCondRuntime* PageState::WireConditional(const CompiledConditional& c) {
    JSContext* ctx = jsRt_->ctx();
    {
        ...
        uint64_t effectId = jsRt_->WatchEffect([this, ctx, rtRaw]() {
            ...
            if (truthy && !isMounted) {
                ...
                // The created runtime is tracked as a child so the unmount
                // path can dispose it before this subtree is freed.
                for (auto& sc : sub.conditionals) {
                    page_.conditionals.push_back(std::move(sc));
                    if (JsCondRuntime* child = WireConditional(page_.conditionals.back()))
                        rtRaw->childConds.push_back(child);
                }
                ...
                ui::RequestLayout();
                // Nested v-ifs wired while this subtree was mounted hold
                // parentWidget/templateNode pointers INTO this subtree.
                // Dispose their watchers + unmount their subtrees BEFORE
                // freeing it — otherwise the stale watchers fire on a later
                // state change and dereference freed widgets (use-after-free).
                TearDownChildConds(rtRaw);
...
// Recursively tear down the nested conditional runtimes wired while `rt`'s
// subtree was mounted. Order matters: children are processed while the
// subtree (and their parents) is still alive, so RemoveChild and unmount
// hooks operate on valid widgets. After this, a child runtime is fully
// neutralized (watchers disposed, closures freed, pointers cleared) and can
// never fire again.
void PageState::TearDownChildConds(JsCondRuntime* rt) {
    if (!rt || !jsRt_) return;
    JSContext* ctx = jsRt_->ctx();
    for (JsCondRuntime* ch : rt->childConds) {
        if (!ch) continue;
        // Grandchildren's parents live inside ch's subtree — recurse first.
        TearDownChildConds(ch);
        if (ch->mounted) {
            DispatchUnmountHooks(ch->mounted.get());
            if (ch->parent) ch->parent->RemoveChild(ch->mounted.get());
            ch->mounted.reset();
        }
        for (auto& lr : ch->innerLoops) if (lr) TearDownJsLoopRuntime(*lr);
        ch->innerLoops.clear();
        ch->innerLoopSpecs.clear();
        for (uint64_t id : ch->innerEffects) jsRt_->DisposeEffect(id);
        ch->innerEffects.clear();
        for (auto& f : ch->innerFns) JS_FreeValue(ctx, f);
        ch->innerFns.clear();
        if (ch->effectId) { jsRt_->DisposeEffect(ch->effectId); ch->effectId = 0; }
        if (!JS_IsUndefined(ch->fn)) { JS_FreeValue(ctx, ch->fn); ch->fn = JS_UNDEFINED; }
        ch->parent = nullptr;
        ch->templateNode = nullptr;
        ch->sheet = nullptr;
        ch->childConds.clear();
    }
    rt->childConds.clear();
...
void PageState::DestroyJsIteration(JsLoopRuntime& rt, JsLoopIteration& iter) {
    ...
    // v-for directly inside this v-for (v-for > v-for) — same rule: the
    // inner runtimes' listFn calls pass this iteration's itemValue, which
    // is about to be freed. Tear them down first.
    for (auto& lr : iter.innerLoops) {
        if (lr) TearDownJsLoopRuntime(*lr);
    }
    iter.innerLoops.clear();
    iter.innerLoopSpecs.clear();
...
void PageState::RewireIterationBindings(JsLoopRuntime& rt, JsLoopIteration& iter) {
    ...
    // rt->outerIter->itemValue at CALL time (already the new value), and
    // the fresh effect runs immediately, refreshing inner iterations.
    for (auto& lr : iter.innerLoops) {
        if (!lr) continue;
        if (lr->watcherId) jsRt_->DisposeEffect(lr->watcherId);
        JsLoopRuntime* lrRaw = lr.get();
        lr->watcherId = jsRt_->WatchEffect([this, lrRaw]() {
            RebuildJsLoop(*lrRaw);
        });
    }
}
...
std::unique_ptr<PageState::JsLoopIteration> PageState::BuildJsIteration(
    ...
    // ---- v-for nested directly inside this iteration (v-for > v-for) ----
    // e.g. the calendar: outer v-for over weeks, inner v-for over days of
    // each week. The compiler collects the inner loops into
    // iter->subPage.loops (skipRootVFor suppresses only the template root);
    // build a JsLoopRuntime for each, compiled against this loop's scope —
    // the inner listFn (`week.days` etc.) gets (loopVar[, indexVar]) as
    // leading params and their values come from iterRaw at call time.
    // This mirrors the v-for > v-if > v-for wiring in BuildCondInIter.
    {
        std::vector<std::string> outerParams;
        if (!loopVar.empty())  outerParams.push_back(loopVar);
        if (!indexVar.empty()) outerParams.push_back(indexVar);
        iter->innerLoopSpecs = std::move(iter->subPage.loops);
        iter->innerLoops.reserve(iter->innerLoopSpecs.size());
        for (auto& spec : iter->innerLoopSpecs) {
            iter->innerLoops.push_back(
                BuildJsLoopRuntimeInScope(&spec, locals, outerParams,
                                          iterRaw, hasIdx));
        }
    }
